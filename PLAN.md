...
- https://gongbaekgi.vercel.app  ·  https://github.com/J-DONGHYUN/gongbaekgi (public)
- **재배포는 `git push` 만 하면 됩니다.** 무중단이고 실패하면 교체되지 않습니다.

### 2차 개선 완료

- 예상 범위를 기준주기 ±30%(최소 ±20일)로 좁힘 — 1년 반짜리 구간이 사라짐
- 신뢰도 기준 완화 (편차/주기 > 1.5) — 전체 → 39팀 중 7팀만 "낮음"
- **30일 병합 규칙** — 30일 안의 연속 발매를 한 활동으로 봄. BTS 주기 23일 → 196일로 정상화
- 베스트 컴필레이션 제외 (`the best`, `best of`, `greatest hits`)
- 컴백 3회 미만 팀을 홈 순위에서 분리해 하단 별도 구획으로
- 페이지별 OG·트위터 메타 (링크 공유 시 미리보기에 숫자가 뜸)
- X 공유 버튼 + 링크 복사 버튼

### 3차 개선 완료 — 팀 색상 + 앨범 커버

- **팀 색상**: 마지막 컴백 앨범 커버에서 **hue 만 추출**해 저장 (39/43팀, 흑백 커버 4팀은 회색)
  이미지를 내려받지도 저장하지도 않으므로 저작권 문제가 없다. 컴백하면 색이 자동으로 바뀐다.
  채도·명도는 CSS 에서 고정 → 어떤 커버든 라이트/다크 양쪽에서 읽힌다.
  **상태색(초록·파랑·주황)은 의미를 담으므로 팀 색은 장식 위치에만** (상단 바·왼쪽 바·배경 틴트)
- **앨범 커버**: 상세 페이지 썸네일(56px) + 이력 표(40px). 핫링크만, 저장 안 함
  [iTunes Search API 약관](https://performance-partners.apple.com/search-api)이 요구하는 조건 준수 —
  아트워크는 **스토어로 가는 링크 옆에** 있어야 한다 → 이미지 전체가 Apple Music 링크
  이미지를 수정·크롭하지 않으며 **OG 이미지에는 넣지 않는다**(합성=재배포에 해당)
- 푸터에 출처 표기 + 권리자 요청 시 삭제 문구
- OG 이미지 상단에 팀 색상 바 10px → 타임라인에서 팀마다 구분됨

### 4차 개선 완료 — 숫자 체감

- `humanDuration()` — 825 → "2년 3개월". **일수와 환산을 함께** 보여준다.
  825 는 추상적이고 2년 3개월은 즉시 와닿아서 둘의 역할이 다르다. 한 달 미만은 환산하지 않는다.
- `stateText()` — 같은 사실을 더 세게 박히는 쪽으로 말한다.
  많이 넘겼으면 **배수**("평균 컴백 주기의 5.3배"), 조금 넘겼으면 **일수**("평균보다 20일 더").
...
- 아티스트 사진 (제공 API 도 사용 허락도 없음 — 캡처해서 올리는 건 더 위험)
- 앨범 커버를 OG 이미지에 합성하는 것 (약관 조건을 벗어난다)
- 수집 자동화 (v3)
