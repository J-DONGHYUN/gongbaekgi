...
export function analyze(comebacks, now = new Date()) {
  ...
  // 예측은 점이 아니라 범위로. 최근 3회 간격의 최소~최대를 마지막 컴백일에 더한다.
  const last3 = gapList.slice(-3);
  const lastMs = new Date(last.date).getTime();
  const forecast = last3.length
    ? {
        from: new Date(lastMs + Math.min(...last3) * DAY),
        to: new Date(lastMs + Math.max(...last3) * DAY),
      }
    : null;
  ...
  return {
    ...
    confidence:
      last3.length < 3
        ? "low"
        : Math.max(...last3) - Math.min(...last3) > cycle
          ? "low"
          : "ok",
  };
}
